...
//		panel_11.add(btnTimKiem);
		...
		btnDD = new JButton("Duyệt đơn ");
		panel_11.add(btnDD);

		btnDD.setVerticalTextPosition(SwingConstants.BOTTOM);
		btnDD.setPreferredSize(new Dimension(90, 85));
		btnDD.setHorizontalTextPosition(SwingConstants.CENTER);
		btnDD.setFont(new Font("Tahoma", Font.BOLD, 14));
		btnDD.setFocusPainted(false);
		btnDD.setBackground(Color.WHITE);
		...
		WarrantyDAO historyDAO = WarrantyDAO.getInstance();
        ArrayList<Warranty> historylist = WarrantyDAO.getInstance().readAllData();
	...
	//     }
	// }
	private void reloadTable(ArrayList<Warranty> invoiceList) {
	    DefaultTableModel model = new DefaultTableModel(
	        new Object[][]{},
	        new String[]{"Mã PBH", "Mã PX", "Lý do", "Trạng thái"}
	    );


	    for (Warranty w : invoiceList) {
	...
	private void addStuff() {
	    ...
	    btnDD.addActionListener(e -> {
//	    	System.out.println(ConfigPRJ.order.get("edit"));

	        int row = table.getSelectedRow();
	        if (row != -1) { 
	            int wid = (Integer) table.getValueAt(row, 1);
	            System.out.println(String.valueOf(wid));
	            boolean success = WarrantyDAO.getInstance().updateStatus(wid);
	            if (success) {
	                JOptionPane.showMessageDialog(null, "Đã duyệt phiếu bảo hành");
	              
	            } else {
	                JOptionPane.showMessageDialog(null, "Lỗi!");
	            }
	        } else {
	            JOptionPane.showMessageDialog(null, "Vui lòng chọn yêu cầu cần duyệt");
	        }
			refresh();
